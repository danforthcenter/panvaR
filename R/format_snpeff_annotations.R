...
#'
#' The `IMPACT_PLUS` column has had modifier snps broken out into intergenic and
#' coding modifiers.
#' @references "Using Drosophila melanogaster as a model for genotoxic chemical
...
format_snpeff_annotations <- function(vcfpath){
  ...
  sort_score <- sort %>% 
    ...
    mutate(Genes.impacted = paste0(unique(.data$GENE), collapse = "|")) %>% 
